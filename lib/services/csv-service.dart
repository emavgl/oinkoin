...
class CSVExporter {
  ...
  static createCSVFromRecordList(
    List<Record?> records, {
    ...
    try {
      ...
      final nonTransferRecords = records
          ...
          .toList();
      _logger.debug(
        'Creating CSV from ${nonTransferRecords.length} records (${records.length - nonTransferRecords.length} transfers excluded)...',
      );

      var recordsMap = List.generate(
        nonTransferRecords.length,
        (index) =>
            nonTransferRecords[index]!.toCsvMap(walletNames: walletNames),
      );
