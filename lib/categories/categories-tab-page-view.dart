...
class CategoryTabPageViewState extends State<CategoryTabPageView> {
  ...
  Future<void> storeOnUserPreferences() async {
    if (_isDefaultOrder) {
      await ServiceConfig.sharedPreferences?.setInt('defaultCategorySortOption', _selectedSortOption.index);
    }
          ...
          builder: (BuildContext context, StateSetter setModalState) {
            return Column(
              ...
              children: [
                ...
                if (_selectedSortOption != SortOption.original) // Show Original Order conditionally
                  ListTile(
  ...
  void _sortByLastUsed() {
    setState(() {
      ...
      _categories?.sort((a, b) {
        ...
        if (aLastUsed == null && bLastUsed == null) return 0; // keep original order
        if (aLastUsed == null) return 1; // 'a' comes after 'b' if 'a' is null
        if (bLastUsed == null) return -1; // 'a' comes before 'b' if 'b' is null

        return bLastUsed.compareTo(aLastUsed); // Regular comparison if both are non-null
      });
    });
    storeOnUserPreferences();
  }
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            _categories != null
                ? CategoriesGrid(
              _categories!.where((element) => element!.categoryType == CategoryType.expense && !element.isArchived).toList(),
              goToEditMovementPage: widget.goToEditMovementPage,
            )
                : Container(),
            _categories != null
                ? CategoriesGrid(
              _categories!.where((element) => element!.categoryType == CategoryType.income && !element.isArchived).toList(),
              goToEditMovementPage: widget.goToEditMovementPage,
            )
                : Container(),
          ],
