...
class HomeWidgetsPageState extends State<HomeWidgetsPage> {
  ...
  Timer? _postPinRefresh;

  @override
  void dispose() {
    _postPinRefresh?.cancel();
    super.dispose();
  }
  ...
  Future<void> _pin(String qualifiedName) async {
    ...
    // The OS may not have registered the new widget instance yet, so the
    // first refresh above can miss it (a pinned budget widget would stay
    // empty until the next refresh). Re-run shortly after to catch it.
    _postPinRefresh?.cancel();
    _postPinRefresh = Timer(const Duration(milliseconds: 900), () async {
      if (!mounted) return;
      await HomeWidgetService.refreshAll();
      await _load();
    });
  ...
  Future<void> _refreshNow() async {
    ...
    await HomeWidgetService.refreshAll();
    // Reload the installed list so widgets added from the launcher show up
    // here as rows (and budget widgets get picked up by auto-assignment).
    await _load();
...
class _BudgetInstanceRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final validBudgetIds = {
      for (final budget in budgets)
        if (budget.id != null) budget.id!,
    };
    return ListTile(
      title: Text("Budget widget".i18n + " #$widgetId"),
      trailing: DropdownButton<int?>(
        // 0 = explicit "None"; a missing assignment shows the hint. A stale
        // id (deleted budget) falls back to the hint instead of crashing.
        value: selectedBudgetId == 0 || validBudgetIds.contains(selectedBudgetId)
            ? selectedBudgetId
            : null,
