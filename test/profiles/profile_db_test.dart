...
void main() {
  ...
  group('Fresh install', () {
    test('creates exactly one Default Profile', () async {
      ...
      expect(
        profiles.where((p) => p.isDefault).length,
        1,
    ...
    test(
      'Default Profile has a non-empty name and is marked isDefault',
      () async {
        final db = ServiceConfig.database;
  ...
  group('Profile CRUD', () {
    ...
    test('addProfile new profile is not marked as default', () async {
      ...
      expect(
        ...
        isFalse,
        reason:
            'Newly created profiles must never become the default '
            'automatically — that would break the single-default invariant',
      );
    });

    test(
      'getAllProfiles returns Default Profile plus any added profiles',
      () async {
        final db = ServiceConfig.database;
    ...
    test(
      'getDefaultProfile always returns the single default profile',
      () async {
        final db = ServiceConfig.database;
        await db.addProfile(Profile('Work'));
  ...
  group('Record isolation between profiles', () {
    ...
    test(
      'records added under Profile A are not visible when querying Profile B',
      () async {
        final db = ServiceConfig.database;
      ...
      """,
        ...
        expect(forB.length, 0, reason: "Bob must not see Alice's records");
      },
    );

    test(
      'records added under Profile B are not visible when querying Profile A',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
      ...
      """,
        ...
        expect(forA.length, 0, reason: "Alice must not see Bob's records");
    ...
    test(
      'records from each profile are correctly separated when both have data',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
    ...
    test(
      'getAllRecords without profileId returns records from all profiles',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
  ...
  group('Wallet isolation between profiles', () {
    ...
    test(
      'wallet added to Profile A is not visible when querying Profile B',
      () async {
        final db = ServiceConfig.database;
        ...
        expect(
          walletsForB.any((w) => w.name == 'Alice Savings'),
          isFalse,
    ...
    test(
      'wallet added to Profile B is not visible when querying Profile A',
      () async {
        final db = ServiceConfig.database;
        ...
        expect(
          walletsForA.any((w) => w.name == 'Bob Cash'),
          isFalse,
    ...
    test(
      'getAllWallets without profileId returns wallets from all profiles',
      () async {
        final db = ServiceConfig.database;
  ...
  group('Recurrent pattern isolation between profiles', () {
    ...
    test(
      'pattern added to Profile A is not visible when querying Profile B',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
      ...
      """,
        ...
        final forA = await db.getRecurrentRecordPatterns(profileId: profileAId);
        final forB = await db.getRecurrentRecordPatterns(profileId: profileBId);
    ...
    test(
      'getRecurrentRecordPatterns without profileId returns all patterns',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
  ...
  group('Profile deletion', () {
    ...
    test(
      'deleteProfileAndRecords removes all records belonging to that profile',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
      ...
      """,
        ...
        final remaining = await raw.query(
          'records',
          where: 'profile_id = ?',
          whereArgs: [profileId],
        );
    ...
    test(
      'deleteProfileAndRecords removes all wallets belonging to that profile',
      () async {
        final db = ServiceConfig.database;
        final profileId = await db.addProfile(Profile('To Delete'));
        ...
        expect(
          await db.getWalletById(walletId),
          isNull,
    ...
    test('deleteProfileAndRecords removes all recurrent patterns belonging to that profile', () async {
      final db = ServiceConfig.database;
      final raw = (await (db as SqliteDatabase).database)!;
    ...
    test(
      'deleteProfileAndRecords does NOT touch records from other profiles',
      () async {
        final db = ServiceConfig.database;
        final raw = (await (db as SqliteDatabase).database)!;
    ...
    test(
      'deleteProfileAndRecords does NOT touch wallets from other profiles',
      () async {
        final db = ServiceConfig.database;
        final toDelete = await db.addProfile(Profile('To Delete'));
        final toKeep = await db.addProfile(Profile('To Keep'));
