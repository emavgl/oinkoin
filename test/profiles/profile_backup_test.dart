...
void main() {
  ...
  testlib.group('Backup creation includes profiles', () {
    ...
    testlib.test(
      'backup JSON includes all profiles when multiple exist',
      () async {
    ...
    testlib.test(
      ...
      () async {
        ...
        final nonDefaults = profiles
            .where((p) => p['is_default'] == 0)
            .toList();
  ...
  testlib.group('Import — modern backup with profiles', () {
    testlib.test(
      'records are assigned to the remapped profile after import',
      () async {
        final db = ServiceConfig.database;
        ...
        final aliceProfile = profiles.firstWhere(
          (p) => p.name == 'Alice Profile',
        );
        ...
        final defaultRecords = await db.getAllRecords(
          profileId: defaultProfileId,
        );
        expect(
          defaultRecords.any((r) => r!.title == 'Alice Rent'),
          isFalse,
          reason: "Alice's record must not bleed into the pre-existing Default Profile",
        );
      },
    );

    testlib.test(
      'records from two different profiles stay separated after import',
      () async {
        final db = ServiceConfig.database;
    ...
    testlib.test(
      'wallets are assigned to the correct remapped profile',
      () async {
        final db = ServiceConfig.database;
        ...
        expect(
          carolWallets.any((w) => w.name == 'Carol Checking'),
          isTrue,
        ...
        final defaultProfile = await db.getDefaultProfile();
        final defaultWallets = await db.getAllWallets(
  ...
  testlib.group('Import — old backup without profiles key', () {
    testlib.test('records from an old backup are assigned to the existing Default Profile', () async {
      final db = ServiceConfig.database;
      final defaultProfile = await db.getDefaultProfile();
      ...
      expect(
        defaultRecords.any((r) => r!.title == 'Old Rent'),
        isTrue,
        reason:
    ...
    testlib.test('old backup import does not create any new profile', () async {
      ...
      expect(
        profilesAfter.length,
        profilesBefore.length,
        reason: 'Importing an old backup must not create new profiles',
      );
    });

    testlib.test('wallets from an old backup are assigned to the Default Profile', () async {
      final db = ServiceConfig.database;
      final defaultProfile = await db.getDefaultProfile();
      ...
      expect(
        defaultWallets.any((w) => w.name == 'Legacy Wallet'),
        isTrue,
        reason:
  ...
  testlib.group('Full backup/restore round-trip with multiple profiles', () {
    testlib.test('after backup and restore, each profile contains only its own records', () async {
      final db = ServiceConfig.database;
      ...
      final restoredDefault = restoredProfiles.firstWhere(
        (p) => p.isDefault && p.name.isNotEmpty,
      );
      final restoredWork = restoredProfiles.firstWhere((p) => p.name == 'Work');

      final defaultRecords = await ServiceConfig.database.getAllRecords(
        profileId: restoredDefault.id,
      );
      final workRecords = await ServiceConfig.database.getAllRecords(
        profileId: restoredWork.id,
      );

      // Each profile must have exactly its own records
      expect(
        defaultRecords.any((r) => r!.title == 'Personal Expense'),
        isTrue,
        reason: 'Default profile record must be restored',
      );
      expect(
        workRecords.any((r) => r!.title == 'Work Expense'),
        isTrue,
      ...
      expect(
        defaultRecords.any((r) => r!.title == 'Work Expense'),
        isFalse,
        reason: 'Work expense must not appear in the Default profile',
      );
      expect(
        workRecords.any((r) => r!.title == 'Personal Expense'),
        isFalse,
  ...
  testlib.group('Restore backup with existing Default Profile', () {
    testlib.test(
      'restoring a backup does NOT create a duplicate Default Profile',
      () async {
        final db = ServiceConfig.database;
        ...
        expect(
          defaultProfileAfter!.id,
          defaultProfileIdBefore,
          reason:
        ...
        final defaultRecords = await db.getAllRecords(
          profileId: defaultProfileAfter.id,
        );
        expect(
          defaultRecords.any((r) => r!.title == 'Lunch'),
          isTrue,
          reason:
  ...
  testlib.group('Profile ID remapping', () {
    testlib.test('imported profiles receive new database IDs, not the backup IDs', () async {
      final db = ServiceConfig.database;
      ...
      expect(
        exotic.id,
        isNot(777),
        reason: 'Profile IDs from the backup file must be remapped to new DB-assigned IDs',
      );
    });
  });
}
