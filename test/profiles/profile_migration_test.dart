...
void main() {
  ...
  group('Migration v22 → v23 with existing data', () {
    ...
    test('migration backfills all pre-existing records with the Default Profile id', () async {
      await SqliteMigrationService.onUpgrade(db, 22, 23);
      ...
      for (final record in records) {
        expect(
          record['profile_id'],
          defaultProfileId,
          reason: 'Every pre-existing record must be assigned to the Default Profile',
        );
      }
    });

    test('migration backfills all pre-existing wallets with the Default Profile id', () async {
      await SqliteMigrationService.onUpgrade(db, 22, 23);
    ...
    test('migration backfills all pre-existing recurrent patterns with the Default Profile id', () async {
      await SqliteMigrationService.onUpgrade(db, 22, 23);
    ...
    test(
      'record count is preserved after migration (no data is lost)',
      () async {
  ...
  group('Migration v22 → v23 with empty tables', () {
    ...
    test(
      'migration still creates one Default Profile even when tables are empty',
      () async {
        await SqliteMigrationService.onUpgrade(db, 22, 23);
  ...
  group('Migration idempotency', () {
    test(
      'running migration twice does not duplicate the Default Profile',
      () async {
        ...
        expect(
          ...
          reason: 'At minimum the Default Profile must exist after a repeated migration',
        );
        await db.close();
      },
    );
