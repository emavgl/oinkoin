...
void main() {
  ...
  test('BUG: importing backup into clean DB causes tags to be assigned to wrong records', () async {
    ...
    for (var record in backup.records) {
      expect(
        ...
        isEmpty,
        reason:
            'Records from backup should have empty tags '
    ...
    expect(
      titleToTags['Groceries at Lidl'],
      contains('Lidl'),
      reason:
          'BUG: "Groceries at Lidl" should have tag "Lidl" '
          'but tags are lost/misassigned due to ID mismatch',
    );
    expect(
      titleToTags['Groceries at Aldi'],
      contains('Aldi'),
      reason: 'BUG: "Groceries at Aldi" should have tag "Aldi"',
    );
    expect(
      titleToTags['Groceries at Colruyt'],
      contains('Colruyt'),
      reason: 'BUG: "Groceries at Colruyt" should have tag "Colruyt"',
    );
  });

  test('BUG: importing backup with overlapping IDs causes tags assigned to WRONG records', () async {
    ...
    expect(
      titleToRecord['Record A']!.tags,
      contains('Tag_for_A'),
      reason:
          'BUG: Record A should have Tag_for_A but it was assigned to Record C',
    );
    expect(
      titleToRecord['Record B']!.tags,
      contains('Tag_for_B'),
      reason:
          'BUG: Record B should have Tag_for_B but it was assigned to Record D',
    );
    expect(
      titleToRecord['Record C']!.tags,
      contains('Tag_for_C'),
      reason:
          'BUG: Record C should have Tag_for_C but it was assigned to Record E',
    );
    expect(
      titleToRecord['Record D']!.tags,
      isEmpty,
      reason: 'BUG: Record D should have no tags but got Tag_for_B',
    );
    expect(
      titleToRecord['Record E']!.tags,
      isEmpty,
  ...
  test(
    'Verify backup JSON matches the database for user-reported data',
    () async {
      ...
      final backupFile = File(
        'debug/piggybankpro_1.4.1_2026-02-14T12-07-48_obackup.json',
      );
      ...
      final tagRecordIds = backup.recordTagAssociations
          .map((a) => a.recordId)
          .toSet();
      ...
      expect(
        ...
        isEmpty,
        reason:
            'All tag associations in the backup should reference '
  ...
  test('Verify records in backup have no tags field (root cause of the bug)', () async {
    ...
    expect(
      ...
      isFalse,
      reason:
          'Record.toMap() does not include tags — '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Tags are lost during serialization round-trip '
          'because toMap() excludes them',
    );
  });

  test(
    'Merge import: existing records keep tags, new records get correct tags',
    () async {
      final DatabaseInterface db = ServiceConfig.database;
  ...
  test('Multiple tags per record are all correctly associated', () async {
    ...
    expect(allRecords.length, 1);
    expect(
      allRecords.first!.tags,
      containsAll(['groceries', 'weekly', 'organic']),
    );
  ...
  test(
    'Backup.fromMap/toMap serialization roundtrip preserves all fields',
    () async {
      final category = Category(
        'Food',
        categoryType: CategoryType.expense,
      ...
      expect(
        restored.recurrentRecordsPattern.length,
        backup.recurrentRecordsPattern.length,
      );
      expect(
        restored.recordTagAssociations.length,
  ...
  test(
    'BackupService.isEncrypted returns true for encrypted/non-JSON file',
    () async {
