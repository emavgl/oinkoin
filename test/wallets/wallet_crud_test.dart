...
void main() {
  ...
  test('deleteWalletAndRecords removes wallet and its records', () async {
    ...
    final records = await rawDb.query(
      'records',
      where: 'wallet_id = ?',
      whereArgs: [walletId],
    );
  ...
  test(
    ...
    """,
      ...
      final recordsInA = await rawDb.query(
        'records',
        where: 'wallet_id = ?',
        whereArgs: [walletA],
      );
      expect(recordsInA, isEmpty);
      final orphanedInB = await rawDb.query(
        'records',
  ...
  test('moveRecordsToWallet deletes self-transfers and updates transfer references', () async {
    DatabaseInterface db = ServiceConfig.database;
    SqliteDatabase sqliteDb = ServiceConfig.database as SqliteDatabase;
    final rawDb = (await sqliteDb.database)!;
    ...
    final selfTransfers = await rawDb.query(
      'records',
    ...
    final movedRecord = await rawDb.query(
      'records',
      where: 'wallet_id = ? AND transfer_wallet_id = ?',
      whereArgs: [walletB, walletC],
    );
    expect(movedRecord.length, 1);
    final partnerRecord = await rawDb.query(
      'records',
      where: 'wallet_id = ? AND transfer_wallet_id = ?',
  ...
  test('moveRecordsToWallet reassigns records', () async {
    ...
    final movedRecords = await rawDb.query(
      'records',
      where: 'wallet_id = ?',
      whereArgs: [toId],
    );
    expect(movedRecords.isNotEmpty, true);
    final fromRecords = await rawDb.query(
      'records',
      where: 'wallet_id = ?',
      whereArgs: [fromId],
    );
  ...
  test('deleteWalletAndRecords deletes orphaned records with NULL wallet_id '
    ...
    // Pattern gone
    final patterns = await rawDb.query(
    ...
    final normalRecords = await rawDb.query(
      'records',
      where: 'wallet_id = ?',
      whereArgs: [walletId],
    );
    ...
    final orphanRecords = await rawDb.query(
      'records',
      where: 'recurrence_id = ?',
      whereArgs: [patternId],
    );
  ...
  test(
    'deleteWalletAndRecords does not delete records from unrelated patterns',
    () async {
      DatabaseInterface db = ServiceConfig.database;
      SqliteDatabase sqliteDb = ServiceConfig.database as SqliteDatabase;
      final rawDb = (await sqliteDb.database)!;
    ...
    """,
      ...
      final otherPatterns = await rawDb.query(
        'recurrent_record_patterns',
        where: 'id = ?',
        whereArgs: [otherPatternId],
      );
      expect(otherPatterns.length, 1);
      final otherRecords = await rawDb.query(
        'records',
        where: 'recurrence_id = ?',
        whereArgs: [otherPatternId],
      );
