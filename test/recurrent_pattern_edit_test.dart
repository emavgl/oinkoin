...
void main() {
  ...
  group('Recurrent pattern editing preserves past records', () {
    test('editing metadata (amount/title) should preserve past records', () async {
    ...
    test('editing with period change should preserve past records', () async {
      ...
      await db.addRecurrentRecordPattern(
    ...
    test('deleteFutureRecordsByPatternId with pattern start date '
  ...
  group('Recurrent pattern wallet update', () {
    ...
    test(
      'changing pattern wallet preserves past records with old wallet',
      () async {
        // Create two wallets
        ...
        for (var r in allRecords) {
          expect(
            r!.walletId,
            walletAId,
            reason: 'Past record must retain original walletA after pattern wallet change',
          );
        }
      },
    );
    ...
    test(
      'newly generated records after wallet change use new wallet',
      () async {
        // Create two wallets
        final walletA = Wallet("Checking", initialAmount: 0);
        final walletB = Wallet("Savings", initialAmount: 0);
        ...
        final walletARecords = allRecords
            .where((r) => r!.walletId == walletAId)
            .toList();
        expect(
          walletARecords.length,
          initialCount,
          reason: 'Past records with walletA should be preserved',
        );

        // Records with walletB should be the newly generated ones
        final walletBRecords = allRecords
            .where((r) => r!.walletId == walletBId)
            .toList();
        expect(
          walletBRecords.length,
          greaterThan(0),
          reason: 'New records should use the updated walletB',
        );
        expect(
          walletBRecords.length,
          allRecords.length - initialCount,
        ...
        for (var r in allRecords) {
          expect(
            r!.walletId,
            anyOf(walletAId, walletBId),
            reason: 'All records must be assigned to either walletA or walletB',
          );
        }
      },
    );
    ...
    test(
      'pattern created without walletId gets default wallet from migration',
      () async {
  ...
  group('Recurrent pattern wallet init', () {
    ...
    test('RecurrentRecordPattern.fromRecord copies walletId', () async {
      ...
      expect(
        pattern.walletId,
        walletId,
        reason: 'fromRecord must copy walletId from the source Record',
      );
    });

    test('RecurrentRecordPattern.fromRecord copies transferWalletId and transferValue', () async {
      ...
      expect(pattern.walletId, srcId, reason: 'fromRecord must copy walletId');
      expect(
        pattern.transferWalletId,
        dstId,
        reason: 'fromRecord must copy transferWalletId',
      );
      expect(
        pattern.transferValue,
        50.0,
    ...
    test(
      'getPredefinedWallet returns correct wallet after setPredefinedWallet',
      () async {
        ...
        expect(
          predefinedA!.id,
          walletBId,
          reason: 'After reassigning, the predefined wallet should be walletB, not walletA',
        );
    ...
    test(
      'new record without wallet uses predefined wallet (not default)',
      () async {
    ...
    test('new record without predefined wallet and only one wallet uses that wallet', () async {
      ...
      expect(
        walletToSelect!.id,
        walletAId,
        reason: 'With one wallet and no predefined, the solo wallet should be selected',
      );
    });
  });
}
